...
let translation = {
    ...
    ar : {
        ...
        orignal : "التقليدي",
        language : "اللغه",
        notify : "هذه المهمه متت اضافتها الي جدول المهام" 
    }
} 
...
function addeletopagefrom(arraytasks){
    ...
    arraytasks.forEach(task => {
        ...
        left.appendChild(document.createTextNode(task.title));
    ...
    const selectedLang = localStorage.getItem('selectedlang');
        if (window.innerWidth <= 993) {
            if (selectedLang === 'en') {
                ...
                sidebar.style.right = ''; // Clear any previous right style
                sidebar.style.transform = 'translateX(-100%)';
            } else if (selectedLang === 'ar') {
                sidebar.style.right = '0';
                sidebar.style.left = ''; // Clear any previous left style
                sidebar.style.transform = 'translateX(100%)';
            }
        } else {
            // Optionally reset styles for larger screens
            sidebar.style.left = '';
            sidebar.style.right = '';
